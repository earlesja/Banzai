...
public class BrowserUsageFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		metricIds.put("Firefox", FIREFOX_METRIC_IDS);
		metricIds.put("Chrome", CHROME_METRIC_IDS);
		metricIds.put("Mobile Chrome", MOBILE_CHROME_METRIC_IDS);
		metricIds.put("Safari", SAFARI_METRIC_IDS);
		metricIds.put("Mobile Safari", MOBILE_SAFARI_METRIC_IDS);
		metricIds.put("Other Browser", OTHER_BROWSER_METRIC_IDS);
		metricIds.put("IE 8", IE_IIX_METRIC_IDS);
		metricIds.put("IE 9", IE_IX_METRIC_IDS);
		metricIds.put("IE 10", IE_X_METRIC_IDS);
		metricIds.put("IE 11", IE_XI_METRIC_IDS);
		...
		TimeFrameChooser mSpinnerCallback = new TimeFrameChooser(
				new Callback() {
					@Override
					public void process(int timeframe, String groupBy,
							DateFormat formatter) {
						setSpinnerVisibility(false);
						BrowserUsageFragment.this.getAndSetChartData(timeframe,
								groupBy, formatter);
					}
				});
		...
		new HistoricalDataTask(timeframe, groupBy, getAllMetricIds(),
				new IGetTaskCallback() {
					...
					public void execute(JSONArray json) {
						...
						for (int i = 0; i < json.length(); i++) {
							try {
								...
								if (CHROME_METRIC_IDS.contains(metricId)) {
									dataSetTitle = "Chrome";
								} else if (MOBILE_CHROME_METRIC_IDS
										.contains(metricId)) {
									dataSetTitle = "Mobile Chrome";
								} else if (FIREFOX_METRIC_IDS
										.contains(metricId)) {
									dataSetTitle = "Firefox";
									// } else if
									// (IE_METRIC_IDS.contains(metricId)) {
									// dataSetTitle = "IE";
									...
									dataSetTitle = "Safari";
								} else if (MOBILE_SAFARI_METRIC_IDS
										.contains(metricId)) {
									dataSetTitle = "Mobile Safari";
								} else if (OTHER_BROWSER_METRIC_IDS
										.contains(metricId)) {
						...
						for (String key : map.keySet()) {
							pieData.put(key,
									map.get(key).get(map.get(key).size() - 1));
						}

						mPieChart.setData(pieData);

						mLineChart.setData(map,
								times.toArray(new Date[times.size()]),
								dateFormatter);
	...
	public void updateContent(ArrayList<String> data) {
		...
		/*
		 * TODO: Make setting values generic instead of being done manually. For
		 * some reason these are the indexes used by the graph
		 * 
		 * index 0 = mobile chrome index 1 = firefox index 2 = mobile safari
		 * index 3 = other browser index 4 = ie 10 index 5 = ie 8 index 6 =
		 * safari index 7 = chrome index 8 = ie 9 index 9 = ie 11
		 */

		this.mPieChart.getData().getDataSet().getEntryForXIndex(0)
				.setVal(metricAverages.get("Mobile Chrome") / total * 100);
		this.mPieChart.getData().getDataSet().getEntryForXIndex(1)
				.setVal(metricAverages.get("Firefox") / total * 100);
		this.mPieChart.getData().getDataSet().getEntryForXIndex(2)
				.setVal(metricAverages.get("Mobile Safari") / total * 100);
		this.mPieChart.getData().getDataSet().getEntryForXIndex(3)
				.setVal(metricAverages.get("Other Browser") / total * 100);
		this.mPieChart.getData().getDataSet().getEntryForXIndex(4)
				.setVal(metricAverages.get("IE 10") / total * 100);
		this.mPieChart.getData().getDataSet().getEntryForXIndex(5)
				.setVal(metricAverages.get("IE 8") / total * 100);
		this.mPieChart.getData().getDataSet().getEntryForXIndex(6)
				.setVal(metricAverages.get("Safari") / total * 100);
		this.mPieChart.getData().getDataSet().getEntryForXIndex(7)
				.setVal(metricAverages.get("Chrome") / total * 100);
		this.mPieChart.getData().getDataSet().getEntryForXIndex(8)
				.setVal(metricAverages.get("IE 9") / total * 100);
		this.mPieChart.getData().getDataSet().getEntryForXIndex(9)
				.setVal(metricAverages.get("IE 11") / total * 100);
