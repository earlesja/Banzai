...
public class PieGraph extends View implements HoloGraphAnimate {
	...
	protected int mInnerCircleRatio;
	...
	public PieGraph(Context context, AttributeSet attrs, int defStyle) {
		...
		mInnerCircleRatio = a.getInt(R.styleable.PieGraph_pieInnerCircleRatio,
				0);
		mPadding = a.getDimensionPixelSize(
				R.styleable.PieGraph_pieSlicePadding, 0);
	}
	...
	public void onDraw(Canvas canvas) {
		...
		for (PieSlice slice : mSlices) {
			...
			mRectF.set(midX - radius, midY - radius, midX + radius, midY
					+ radius);
			createArc(p, mRectF, currentSweep, currentAngle + mPadding,
	...
	private void createArc(Path p, RectF mRectF, float currentSweep,
